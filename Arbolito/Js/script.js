...
fetch('Img/treelove.svg')
  ...
  .then(svgText => {
    ...
    const heartPaths = allPaths.filter(el => {
      ...
      return style.includes('#9758fc') || style.includes('#681fc1');
    });
...
function showDedicationText() { 
  ...
  if (!text) {
    text = text = `Ola mi amor:\n\ncosas random en las que traveseo, ojalá esto te parezca bonito, pero bueno, quise hacer algo diferente y original en caso de no poder darte algo el 14.\n\nComo cada día que paso con vos es increíble, quise ponerme a contar cuántos días increíbles he tenido desde que estoy con vos.\n\nTe amo`; } else {
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "Con amor, Armandito";
...
function showCountdown() {
  ...
  let startDate = startParam ? new Date(startParam + 'T00:00:00') : new Date('2023-10-25T00:00:00'); 
  let eventDate = eventParam ? new Date(eventParam + 'T00:00:00') : new Date('2026-10-25T00:00:00');
  ...
  function update() {
    ...
    container.innerHTML =
      `Dias Increibles: <b>${days}</b> días<br>` +
